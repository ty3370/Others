...
# 사용법 섹션
st.subheader("📖 시뮬레이션 사용법")
st.markdown("""
1. **고도 조절**: 마우스를 화면 위아래로 움직여 공기 덩어리를 상승시키거나 하강시켜 보세요.
2. **기온 감률($\Gamma$) 확인**: 감률 슬라이더를 높이면 고도가 올라갈 때 **외부 화살표(압력)**가 훨씬 빨리 작아지고 공기가 더 크게 팽창합니다.
3. **단열 지수($\gamma$) 확인**: 지수가 높을수록 상승 시 공기의 **온도(색상)**가 더 급격하게 파란색으로 변합니다.
4. **비평형 관찰**: 상승할수록 외부 화살표는 작아지지만 내부 화살표는 상대적으로 길게 유지됩니다. 이 **길이 차이**가 공기를 팽창시키는 원동력임을 확인하세요.
""")